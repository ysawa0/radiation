...
% [distanceMean, overlap, doseMean, emd, dvh] = mysql(horzcat('SELECT DistanceMean_ptv1, OverlapVolume_ptv1, doseMean FROM structure_set_roi_sequence_copy WHERE stdROIName="',roi,'" AND fk_patient_id IN (',patientsString,')'));
...
[o,d]=mysql(horzcat('SELECT emd, dvhDistance FROM ',roi));
[ovhSorted,sortedIndices]=sort(o);
dvhSorted=d(sortedIndices);

plot(1:length(o),ovhSorted,'black');
hold on;
plot(1:length(o),dvhSorted,'blue');

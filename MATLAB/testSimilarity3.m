...
for queryIndex = 1:length(query_patient_ids)
    ...
    [queryDistance queryVolume qAlpha qBeta qGamma qOverlap qDistanceMean qDoseMean qDoseMax, qPrescriptionDose] = mysql(horzcat('SELECT ovhDistance_ptv1, ovhVolume_ptv1, alpha_ptv1, beta_ptv1, gamma_ptv1, PercentageOverlap_ROI_Fraction_ptv1, distanceMean_ptv1, doseMean, doseMax, dosePrescription FROM structure_set_roi_sequence_copy WHERE fk_patient_id="',num2str(query_patient_ids(queryIndex)),'" AND stdROIName="',roiName,'"'));

...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%READING THE EXCEL DATA VALUES%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
fprintf(1,'\n\n\n\n\n\n\n###########################################################');
fprintf(1,'\nMATLAB ROUTINE FOR PROCESSING H&N DVH DATA');
%datadose=xlsread('\\rbrofs1\PHI\Shared\UCLARadoncRegistry\H&N\HNTechData2.xls','Data_Dose'); %Hussain analysis
datadose=xlsread('\\rbrofs1\PHI\Shared\UCLARadoncRegistry\H&N\DVHTechData_H&N.xls','Data_Dose'); %All Patients
[rowdose,coldose] = size(datadose);
fprintf(1,'\n###########################################################');
fprintf(1,'\n# of entries in the DATADOSE excel sheet: %d',rowdose);
%datatech=xlsread('\\rbrofs1\PHI\Shared\UCLARadoncRegistry\H&N\HNTechData2.xls','Data_Tech'); %Hussain analysis
datatech=xlsread('\\rbrofs1\PHI\Shared\UCLARadoncRegistry\H&N\DVHTechData_H&N.xls','Data_Tech'); %All Patients
[rowtech,coltech] = size(datatech);
fprintf(1,'\n# of patients currently listed in the DATATECH excel sheet: %d',rowtech);
S(1) = rowtech;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
c1 = datadose(:,3)== 33;   %Lt Parotid %command gives a vector of 1s and 0s
c2 = datadose(:,3)== 34;   %Rt Parotid
c3 = datadose(:,3)== 35;   %Mandible
...
idx1=find(c1);%Finds the indices of non-zero entries
idx2=find(c2);
idx3=find(c3);
idx4=find(c4);
idx5=find(c5);
...
idxvals=setxor(datatech(:,2),b);%returns all the duplicate entries (the values themselves)
display(b);
fprintf(1,'\n###########################################################');
fprintf(1,'\n###########################################################');
fprintf(1,'\nLT PAROTID ');
[temp] = qc_dvhData(idx1(:), rowsize, datadose, datatech, ib);
fprintf(1,'\nRT PAROTID ');
[temp] = qc_dvhData(idx2(:), rowsize, datadose, datatech, ib);
fprintf(1,'\nMANDIBLE ');
[temp] = qc_dvhData(idx3(:), rowsize, datadose, datatech, ib);
fprintf(1,'\nLT COCHLEA ');
[temp] = qc_dvhData(idx4(:), rowsize, datadose, datatech, ib);
fprintf(1,'\nRT COCHLEA ');
[temp] = qc_dvhData(idx5(:), rowsize, datadose, datatech, ib);
fprintf(1,'\nSPINAL CORD ');
[temp] = qc_dvhData(idx6(:), rowsize, datadose, datatech, ib);
fprintf(1,'\nOPTIC CHIASM ');
[temp] = qc_dvhData(idx7(:), rowsize, datadose, datatech, ib);
fprintf(1,'\nPTV1 ');
[temp] = qc_dvhData(idx8(:), rowsize, datadose, datatech, ib);
fprintf(1,'\nPTV2 ');
[temp] = qc_dvhData(idx9(:), rowsize, datadose, datatech, ib);
fprintf(1,'\nPTV3 ');
[temp] = qc_dvhData(idx10(:), rowsize, datadose, datatech, ib);
fprintf(1,'\n###########################################################');
fprintf(1,'\n###########################################################');
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
mean_ltpar = mean(datadose(idx1(:),4));%idx1 gives the row indices of all the entries for the left parotid 
std_meanltpar = std(datadose(idx1(:),4));
max_ltpar = mean(datadose(idx1(:),5));
...
fprintf(1,'\n###########################################################');
fprintf(1,'\nMean Lt Parotid Dose: %6.1f +/- %6.1f',mean_ltpar,std_meanltpar);
fprintf(1,'\nMax Lt Parotid Dose: %6.1f +/- %6.1f',max_ltpar,std_maxltpar);
fprintf(1,'\nMean Rt Parotid Dose: %6.1f +/- %6.1f',mean_rtpar,std_meanrtpar);
fprintf(1,'\nMax Rt Parotid Dose: %6.1f +/- %6.1f',max_rtpar,std_maxrtpar);
fprintf(1,'\nMean Mandible Dose: %6.1f +/- %6.1f',mean_mand,std_meanmand);
fprintf(1,'\nMax Mandible Dose: %6.1f +/- %6.1f',max_mand,std_maxmand);
fprintf(1,'\nMean Lt Cochlea Dose: %6.1f +/- %6.1f',mean_ltcoch,std_meanltcoch);
fprintf(1,'\nMax Lt Cochlea Dose: %6.1f +/- %6.1f',max_ltcoch,std_meanltcoch);
fprintf(1,'\nMean Rt Cochlea Dose: %6.1f +/- %6.1f',mean_rtcoch,std_meanrtcoch);
fprintf(1,'\nMax Rt Cochlea Dose: %6.1f +/- %6.1f',max_rtcoch,std_maxrtcoch);
fprintf(1,'\nMean Chiasm Dose: %6.1f +/- %6.1f',mean_chiasm,std_meanchiasm);
fprintf(1,'\nMax Chiasm Dose: %6.1f +/- %6.1f',max_chiasm,std_maxchiasm);
fprintf(1,'\nMean Cord Dose: %6.1f +/- %6.1f',mean_cord,std_meancord);
fprintf(1,'\nMax Cord Dose: %6.1f +/- %6.1f',max_cord,std_maxcord);
fprintf(1,'\n###########################################################');
fprintf(1,'\nMean PTV1 Dose: %6.1f +/- %6.1f',mean_ptv1,std_meanptv1);
fprintf(1,'\nMax PTV1 Dose: %6.1f +/- %6.1f',max_ptv1,std_maxptv1);
fprintf(1,'\nMin PTV1 Dose: %6.1f +/- %6.1f',min_ptv1,std_minptv1);
fprintf(1,'\n###########################################################');
fprintf(1,'\nMean PTV2 Dose: %6.1f +/- %6.1f',mean_ptv2,std_meanptv2);
fprintf(1,'\nMax PTV2 Dose: %6.1f +/- %6.1f',max_ptv2,std_maxptv2);
fprintf(1,'\nMin PTV2 Dose: %6.1f +/- %6.1f',min_ptv2,std_minptv2);
fprintf(1,'\n###########################################################');
fprintf(1,'\nMean PTV3 Dose: %6.1f +/- %6.1f',mean_ptv3,std_meanptv3);
fprintf(1,'\nMax PTV3 Dose: %6.1f +/- %6.1f',max_ptv3,std_maxptv3);
fprintf(1,'\nMin PTV3 Dose: %6.1f +/- %6.1f',min_ptv3,std_minptv3);
fprintf(1,'\n###########################################################');
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%idx_male = find(datatech(:,8)== 'MALE');
%idx_female = find(datatech(:,8)== 'FEMALE');
%fprintf(1,'\n# OF MALES: %d',[a1size,colsize] == size(idx_male));
%fprintf(1,'\n# OF FEMALES: %d',[a1size,colsize] == size(idx_female));

%[a2size,colsize] = size(a2_idx);
%fprintf(1,'\nTech = 2 (Brainscan): %d',a2size);
%[a3size,colsize] = size(a3_idx);
fprintf(1,'\n%s',datatech(2,8));
%fprintf(1,'\nTOTAL # OF PATIENTS (TECH SHEET): %d',a1size+a2size+a3size);
%fprintf(1,'\n###########################################################');

fprintf(1,'\n###########################################################');
fprintf(1,'\nIDX1 = LT PAROTID (33) IDX2 = RT PAROTID (34)');
fprintf(1,'\nIDX3 = MANDIBLE (35)');
fprintf(1,'\nIDX4 = LT COCHLEA (36) IDX5 = RT COCHLEA (37) ');
fprintf(1,'\nIDX6 = CORD (7) ');
fprintf(1,'\nIDX7 = CHIASM (6) ');
fprintf(1,'\nIDX8 = PTV1 (30) IDX9 = PTV2 (31) IDX10 = PTV3 (32)');
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
fprintf(1,'\n###########################################################');
%[c1, a1, b1] = intersect( datadose(idx8,1) , datatech(:,1) ); %intersect dose=PTV1 and tech=Eclipse
%[c2, a2, b2] = intersect( datadose(idx9,1) , datatech(:,1) ); %intersect dose=PTV2 and tech=Eclipse
%[c3, a3, b3] = intersect( datadose(idx10,1) , datatech(:,1) ); %intersect dose=PTV3 and tech=Eclipse
%fprintf(1,'\nMEAN PTV1 DOSE (Eclipse Plan): %6.1f',mean(datadose(idx8(a1),4)));
%fprintf(1,'\nMAX PTV1 DOSE (Eclipse Plan): %6.1f',mean(datadose(idx8(a1),5)));
%fprintf(1,'\nMIN PTV1 DOSE (Eclipse Plan): %6.1f',mean(datadose(idx8(a1),6)));
%fprintf(1,'\n###########################################################');
%[c1, a1, b1] = intersect( datadose(idx8,1) , datatech(a2_idx,1) ); %intersect dose=PTV1 and tech=BScan
%[c2, a2, b2] = intersect( datadose(idx9,1) , datatech(a2_idx,1) ); %intersect dose=PTV2 and tech=BScan
%[c3, a3, b3] = intersect( datadose(idx10,1) , datatech(a2_idx,1) ); %intersect dose=PTV3 and tech=BScan
%fprintf(1,'\nMEAN PTV1 DOSE (BScan Plan): %6.1f',mean(datadose(idx8(a1),4)));
%fprintf(1,'\nMAX PTV1 DOSE (BScan Plan): %6.1f',mean(datadose(idx8(a1),5)));
%fprintf(1,'\nMIN PTV1 DOSE (BScan Plan): %6.1f',mean(datadose(idx8(a1),6)));

fprintf(1,'\n###########################################################');
[~, ia, ib] = intersect( datadose(idx8(:),1), datatech(:,1));
[rowsize,~] = size(ib);
...
scatX = datadose(idx3(:),1); %mandible
scatY = datadose(idx3(:),5); %rt cochlea
scatter(scatX,scatY,'filled'); hold on;
...
for i=1:V70GYsize %print patient index and the dose to 0.1% of mandible
    fprintf(1,'\nPATIENT INDEX: %d',datadose(idx3(V70GY_idx(i)),1));
    fprintf(1,'\t%6.1f cGy',datadose(idx3(V70GY_idx(i)),109));  
end
V70GY_MANDIBLE = zeros(V70GYsize,1);
for j = 1:V70GYsize
    xinterp = datadose(idx3(V70GY_idx(j)),9:111);
    [bb,ii,~] = unique(xinterp); 
    V70GY_MANDIBLE(j) = interp1(bb,yinterp(ii),7000);
end
...
Vexceed = datadose(idx7(:),109) > 5500; %Max dose > 5500 cGy %109 represents D0.1%
